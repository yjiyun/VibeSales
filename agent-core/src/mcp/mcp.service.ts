...
export class McpService {
 ...
 private async callP1(name:string,a:Record<string,any>,ctx:TaskContext){const run=await this.store.getRun(ctx.runId);if(run.current_phase!==ProductPhase.P1_WIZARD_INTENT||!['RUNNING','WAITING_HUMAN'].includes(run.status))throw new Error('P1 run is not active');await this.store.updateRun(ctx.runId,{status:'RUNNING'});
  ...
  const triage=this.objectArgument<Triage>(supplied?.triage??a.triage,'triage'),patch=name==='revise'?this.objectArgument<Record<string,unknown>>(a.patch,'patch'):undefined;const source=name==='revise'?{...triage,...patch,known_slots:{...(triage.known_slots??{}),...((patch?.known_slots as Record<string,unknown>|undefined)??{})}}:triage;const gated=this.wizard.evaluateGate(source as Triage);const out=name==='buildPhase1Result'?{...(supplied??{}),phase:'P1',client_code:ctx.clientCode,request_id:ctx.requestId,stage:supplied?.stage??a.stage??'S1_COARSE',gate:gated.gate,triage:gated.triage,ask_user:gated.ask_user}:gated;await this.store.putArtifact(ctx.runId,'wizard_state',out,'wizard-intent');if(name==='buildPhase1Result'&&gated.triage)await this.store.putArtifact(ctx.runId,'triage',gated.triage,'wizard-intent');if(name!=='buildPhase1Result'){await this.store.updateRun(ctx.runId,{status:gated.gate==='ASK'?'WAITING_HUMAN':'RUNNING'});return out;}if(gated.gate==='PASS')await this.store.updateRun(ctx.runId,{status:'RUNNING',current_phase:ProductPhase.P2_TEMPLATE_MATCH});else await this.store.updateRun(ctx.runId,{status:gated.gate==='ASK'?'WAITING_HUMAN':'ABORTED'});return out;}

 private async callP2(name:string,a:Record<string,any>,ctx:TaskContext){if(name==='preview'){const match=await this.requireArtifact<MatchResult>(ctx.runId,'match_result');if(match.action!=='hit'||!match.template_id||match.template_id!==String(a.templateId))throw new Error('preview requires persisted matching hit');return this.preview.build(match.template_id);}const run=await this.store.getRun(ctx.runId);if(run.current_phase!==ProductPhase.P2_TEMPLATE_MATCH||run.status!=='RUNNING')throw new Error('P2 run is not active');const state=await this.requireArtifact<any>(ctx.runId,'wizard_state');if(state.phase!=='P1'||state.gate!=='PASS'||state.client_code!==ctx.clientCode)throw new Error('P2 requires complete persisted P1 PASS');const triage=await this.requireArtifact<Triage>(ctx.runId,'triage');const tenant=this.tenant.resolve(ctx.clientCode,a.tenantPath);const result=await this.match.run({client_code:tenant.client_code,tenant,request_id:ctx.requestId},triage);await this.store.putArtifact(ctx.runId,'match_result',result,'template-match');const path=this.wizard.decideBuildPath(triage,result);if(path==='EARLY_EXIT'){await this.store.updateRun(ctx.runId,{build_path:path,status:'ABORTED',current_phase:ProductPhase.P2_TEMPLATE_MATCH});return{...result,build_path:path,status:'ABORTED'};}await this.store.updateRun(ctx.runId,{build_path:path,current_phase:path==='P3'?ProductPhase.P3_TEMPLATE_PERSONALIZE:path==='P3B'?ProductPhase.P3B_FLOW_GENERATE:ProductPhase.P3C_BLUEPRINT_COMPOSE});return{...result,build_path:path};}

 private async callP3(name:string,a:Record<string,any>,ctx:TaskContext){if(name==='deriveGuidance'){const run=await this.store.getRun(ctx.runId);if(!['P3','P3B','P3C'].includes(String(run.build_path))||run.status!=='RUNNING')throw new Error('guidance requires active build path');const guidance=this.p3.deriveGuidance(await this.requireArtifact(ctx.runId,'triage'));await this.store.putArtifact(ctx.runId,'guidance',guidance,'template-personalize');return guidance;}await this.requirePath(ctx.runId,'P3');if(name==='injectSections'){const pkg=this.p3.personalize(await this.requireArtifact(ctx.runId,'match_result'),await this.guidance(ctx,'template-personalize'));await this.store.putArtifact(ctx.runId,'personalized_package',pkg,'template-personalize');return pkg;}const pkg=await this.requireArtifact<any>(ctx.runId,'personalized_package');const check=this.p3b.selfcheck(pkg);await this.store.putArtifact(ctx.runId,'flow_check',check,'template-personalize');if(!check.ok)throw new Error('P3 selfcheck failed');await this.readyForP4(ctx.runId);return {package:pkg,check};}

 private async callP3b(name:string,a:Record<string,any>,ctx:TaskContext){await this.requirePath(ctx.runId,'P3B');if(name==='generate'){const pkg=this.p3b.generate(await this.requireArtifact(ctx.runId,'triage'),await this.requireArtifact(ctx.runId,'guidance'));await this.store.putArtifact(ctx.runId,'flow_yaml',pkg,'flow-generate');return pkg;}const pkg=await this.requireArtifact<any>(ctx.runId,'flow_yaml');const check=this.p3b.selfcheck(pkg);await this.store.putArtifact(ctx.runId,'flow_check',check,'flow-generate');if(!check.ok)throw new Error('P3B selfcheck failed');await this.readyForP4(ctx.runId);return check;}

 private async callP3c(name:string,a:Record<string,any>,ctx:TaskContext){await this.requirePath(ctx.runId,'P3C');if(name==='listSkillCandidates')return this.p3c.listSkillCandidates(a.industry,a.scenarios??[]);if(name==='listToolCandidates')return this.p3c.listToolCandidates(ctx.clientCode);if(name==='renderPersona')return this.p3c.renderPersona(this.asObject(a.guidance,'guidance') as unknown as Guidance);if(name==='composeBlueprint'){const triage=await this.requireArtifact<Triage>(ctx.runId,'triage'),guidance=await this.requireArtifact<Guidance>(ctx.runId,'guidance'),batchId=randomUUID();// 四个专家产物常以 JSON 字符串传入；不解析的话 Array.isArray(skills) 为 false 会静默退回
...
const expertInputs=[['persona-expert',persona],['business-expert',business],['skill-expert',skills],['tool-expert',tools]] as const;if(expertInputs.some(([,payload])=>payload==null))throw new Error('P3C requires four expert results');await this.store.putArtifact(ctx.runId,'expert_dispatch',{batchId,roles:expertInputs.map(([role])=>role)},'blueprint-compose');for(const[role,payload]of expertInputs)await this.store.putArtifact(ctx.runId,'expert_result',{role,batchId,payload},role);return this.p3c.composeBlueprint({runId:ctx.runId,clientCode:ctx.clientCode,triage,guidance,experts:{persona,business,skills,tools}});}if(name==='blueprintSelfcheck'){const check=await this.p3c.blueprintSelfcheck(this.requireBlueprint(a.blueprint));await this.store.putArtifact(ctx.runId,'blueprint_check',check,'blueprint-compose');return check;}const bp=this.requireBlueprint(a.blueprint);const record=await this.p3c.persistBlueprint(ctx.runId,bp);await this.store.putArtifact(ctx.runId,'blueprint',record.payload,'blueprint-compose');await this.readyForP4(ctx.runId);return record;}
 ...
 private async callP4(name:string,a:Record<string,any>,ctx:TaskContext){
  ...
  if(name==='import'){
   ...
   if(!a.approval){
    if(pending?.payload?.status==='PENDING')return{status:'pending_approval',approval_id:pending.payload.approval_id,run_id:ctx.runId};
   ...
   if(proof.decision==='DENY'){await this.store.putArtifact(ctx.runId,'evidence',{event:'APPROVAL_DENIED',actor:proof.actor,approval_id:proof.approval_id,at:new Date().toISOString()},'flow-import-run');await this.store.updateRun(ctx.runId,{status:'ABORTED'});return{status:'ABORTED',run_id:ctx.runId};}
   await this.store.updateRun(ctx.runId,{status:'RUNNING',current_phase:ProductPhase.P4_IMPORT_RUN});
   try{const{payload,check}=await this.p4Input(ctx.runId,run.build_path);const imported=await this.p4.import({runId:ctx.runId,clientCode:ctx.clientCode,path:run.build_path,payload,check});await this.store.putArtifact(ctx.runId,'import_result',{imported},'flow-import-run');await this.store.transitionApproval(ctx.runId,proof.approval_id,'PROCESSING',{...pending.payload,status:'CONSUMED',actor:proof.actor,consumed_at:new Date().toISOString()});return imported;}catch(error){return this.failP4(ctx.runId,'IMPORT',error);}
  }
  if(name==='bindProject'){
