...
const base='http://127.0.0.1:18001',servers={
 'chatflows-p1':['worker-wizard-intent'],'chatflows-p2':['worker-template-match'],'chatflows-p3':['worker-template-personalize'],
 'chatflows-p3b':['worker-flow-generate'],'chatflows-p3c':['worker-blueprint-compose','worker-persona-expert','worker-business-expert','worker-skill-expert','worker-tool-expert'],
...
async function main(){
 ...
 const consumerReply=await request('/v1/consumers',{headers}),consumers=new Set((consumerReply.data??[]).map(x=>x.name));if(!consumers.has('manager'))throw new Error('Higress manager consumer missing');
 for(const [server,workers] of Object.entries(servers)){
  const allowed=['manager',...workers];for(const consumer of allowed)if(!consumers.has(consumer))throw new Error(server+' consumer missing: '+consumer);
  const mcpReply=await request('/v1/mcpServer/'+server,{headers}),mcp=mcpReply.data;if(!mcp)throw new Error(server+' MCP route missing');mcp.consumerAuthInfo={enable:true,type:'key-auth',strategyConfigId:null,allowedConsumers:allowed};
 ...
 process.stdout.write('[PASS] Higress enforces per-consumer MCP auth and injects a separate Nest upstream token for 6 routes\n');
}
